      ...
      <div className="hero__content">
        <div className="animate-fade-up delay-1">
          <span className="badge badge-amber hero__badge">
            ...
            CSUMB Capstone - Computer Science
          </span>
        </div>
        ...
        <span className="hero__title-sub animate-fade-up delay-2">
          Open Text Transcription Editing Resource
        </span>

        <p className="hero__subtitle animate-fade-up delay-3">
          OTTER uses automatic speech recognition to synchronize audio waveforms with editable transcripts. Runs entirely on your machine — no cloud, no data leaks, no closed-source dependencies.
        </p>
        ...
        <div className="hero__actions animate-fade-up delay-4">
          ...
          <a href="https://github.com/OTTER-Capstone-ORG/OTTER" target="_blank" rel="noopener noreferrer" className="btn btn-ghost">
        ...
        <div className="hero__waveform animate-fade-up delay-5" aria-hidden="true">
          <div className="hero__waveform-track">
            {Array.from({ length: 72 }, (_, i) => (
              <div key={i} className="hero__bar" style={{
                height: `${Math.max(10, Math.abs(Math.sin(i * 0.42 + 1.1) * 44 + Math.cos(i * 0.18) * 20) + 6)}px`,
                animationDelay: `${(i % 12) * 0.09}s`,
                opacity: i > 16 && i < 40 ? 1 : 0.4,
              }} />
            ))}
          </div>
          <div className="hero__waveform-label">
            {['The','quick','brown','fox','jumps','over'].map((w, i) => (
              <span key={w} className={`hero__transcript-word${i === 1 ? ' hero__transcript-word--active' : ''}`}>{w}</span>
            ))}
